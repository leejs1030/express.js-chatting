import { SocialDAO } from '../../main/DAO';
...
describe('Test SocialDAO', async () =>{
    it('canSendRequest, newRequest, cancelRequest', async () =>{
        ...
        try{
            ...
            expect(await SocialDAO.canSendRequest(cur.id, subadmin.id)).true; // 처음엔 당연히 요청 가능.
            expect(await SocialDAO.canSendRequest(cur.id, admin.id)).true; // 얘는 변화 없으니 쭉 블랙 가능이어야 함
            await SocialDAO.newRequest(cur.id, subadmin.id); // 요청 전송
            expect(await SocialDAO.canSendRequest(cur.id, subadmin.id)).false; // 요청 전송 했으니 재요청 불가
            expect(await SocialDAO.canSendRequest(cur.id, admin.id)).true;
            await SocialDAO.cancelRequest(cur.id, subadmin.id); // 요청 삭제(거절/취소)
            expect(await SocialDAO.canSendRequest(cur.id, subadmin.id)).true; // 이젠 다시 요청 가능
            expect(await SocialDAO.canSendRequest(cur.id, admin.id)).true;
        } catch(err){
            throw err;
        } finally{
    ...
    it('canAddBlack, newBlack, unBlack', async () =>{
        ...
        try{
            await createUser(cur);
            expect(await SocialDAO.canAddBlack(cur.id, subadmin.id)).true; // 처음엔 당연히 블랙 가능.
            expect(await SocialDAO.canAddBlack(cur.id, admin.id)).true;
            await SocialDAO.newBlack(cur.id, subadmin.id); // 블랙
            expect(await SocialDAO.canAddBlack(cur.id, subadmin.id)).false; // 블랙 불가
            expect(await SocialDAO.canAddBlack(cur.id, admin.id)).true;
            await SocialDAO.unBlack(cur.id, subadmin.id); // 언블랙
            expect(await SocialDAO.canAddBlack(cur.id, subadmin.id)).true; // 블랙 가능
            expect(await SocialDAO.canAddBlack(cur.id, admin.id)).true;
        } catch(err){
            throw err;
        } finally{
    ...
    it('isFriend, allowRequest, deleteFriend', async () =>{
        ...
        try{
            ...
            expect(await SocialDAO.isFriend(a.id, b.id)).false;
            await SocialDAO.newRequest(a.id, b.id); await SocialDAO.allowRequest(a.id, b.id);
            expect(await SocialDAO.isFriend(a.id, b.id)).true;
    ...
    it('getSocialsById', async () =>{
        ...
        try{
            ...
            expect(friendlist[0].id).not.to.equal(friendlist[1].id); // 서로 다른 애들이랑 친구에다가
            expect(friendlist.length).to.equal(2); // 총 친구는 두 명
            ...
            expect(blacklist[0].id).to.equal(a.id); expect(blacklist.length).to.equal(1);// b가 블랙했었으니, b는 블랙리스트에 a만 있음
